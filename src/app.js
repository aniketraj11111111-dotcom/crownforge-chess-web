...
// Orientation is intentionally hard-locked: White is always at the bottom.
// There is no auto-rotate state, manual rotate button, or mutation path.
...
function render() {
  ...
  for (const square of visualSquares()) {
    ...
    if (legalTargets.has(square.index)) cell.classList.add(piece ? "capture-target" : "legal-target");
  ...
  substatusEl.textContent = `Move ${game.position.fullmoveNumber} · ${game.moveHistory.length} half-moves · Board locked`;
...
function choosePromotion(candidates) {
  return new Promise(resolve => {
    const cleanup = () => promotionDialog.querySelectorAll("[data-promotion]").forEach(button => button.onclick = null);
...
async function onSquare(square) {
  ...
  if (!selected) {
    if (piece?.side === game.position.sideToMove) {
      selected = square;
      selectedMoves = game.getLegalMoves().filter(move => move.from.index === square.index);
      render();
    }
    return;
  }
  ...
  const candidates = selectedMoves.filter(move => move.to.index === square.index);
...
async function showTerminal() {
  ...
  const winner = game.outcome.winner === null
    ? null
    : game.outcome.winner === Side.White ? "WHITE" : "BLACK";
